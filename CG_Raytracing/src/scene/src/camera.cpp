...
Camera::Camera(uint32_t _sensor_size_width, uint32_t _focal_length,
    ...
    : m_sensor_size_width(_sensor_size_width),
      ...
      m_direction(_direction[0], _direction[1], _direction[2]), m_threads{} {
    ...
    for (uint32_t y = 0; y < this->m_image_height; y++) {
        for (uint32_t x = 0; x < this->m_image_width; x++) {
            for (uint32_t ray_index = 0; ray_index < Config::RAY_PER_PIXEL;
                 ray_index++) {
                auto ray_direction = math::GetRandomVecAroundPoint(
                    math::Vec3(x, y, m_position.z), top_left,
                    horizontal_offset.x, vertical_offset.y);
                ray_direction.Rotate(this->m_direction);
                this->m_rays_matrix[(y * this->m_image_width + x) *
                                        Config::RAY_PER_PIXEL +
                                    ray_index]
                    .SetDirection(ray_direction);
                this->m_rays_matrix[(y * this->m_image_width + x) *
                                        Config::RAY_PER_PIXEL +
                                    ray_index]
                    .SetOrigin(this->m_position);
            }
            // math::Vec3 ray_direction =
            //     top_left + horizontal_offset * x + vertical_offset * y;
            // ray_direction.Rotate(this->m_direction);
            // this->m_rays_matrix[(y * this->m_image_width + x) *
            // Config::RAY_PER_PIXEL].SetDirection(
            //     ray_direction);
            // this->m_rays_matrix[(y * this->m_image_width + x) *
            // Config::RAY_PER_PIXEL].SetOrigin(
            //     this->m_position);
        }
    }
    ...
    for (auto i : std::views::iota(0U, num_threads)) {
        ...
        th_data.th = std::jthread(
            [this, i](std::stop_token tok) { this->RenderThreadMain(tok, i); });
...
void Camera::RenderThreadRenderBlock(RenderThreadData const &_data,
                                     RenderParam _param) {
    ...
    // std::println(std::cout, "From X: {}, Y: {} to X: {}, Y: {}", start_x,
    // start_y, end_x, end_y);
    ...
    for (uint32_t y = start_y; y < end_y; y++) {
        for (uint32_t x = start_x; x < end_x; x++) {
            ...
            for (uint32_t ray_index = 0; ray_index < Config::RAY_PER_PIXEL;
                ...
                for (auto curr_iteration :
                     std::views::iota(0U, Config::RENDER_ITERATION)) {
                    // Stack of rays, together with the iteration number
                    // std::stack<std::pair<math::Ray, size_t>>
                    // rays_to_follow{}; rays_to_follow.push({ ray, 0 });
                    ...
                    while (true) {
                        ...
                        if (hit) {
                            hit->m_point =
                                math::Ray(hit->m_point, hit->m_normal)
                                    .At(geometry::Hittable::TMIN * 1.01f);
                            auto scattered =
                                hit->m_material->Scatter(curr_ray, hit.value());

                            if (!scattered.has_value()) {
                                // Check if the surface is emissive — if so,
                                // accumulate its emission. Otherwise treat as
                                // total absorption (e.g. a black hole
                                // material).
                                if (hit->m_material->IsEmissive()) {
                                    // Shade with no external light — emissive
                                    // materials return their own emission color
                                    // from Shade()
                    ...
                    // final_color = final_color * math::Vec3(1.f, ((1.0f - t) *
                    // 180 + t * 80) / 255.f, 1.f);
...
void Camera::BurstRays(/*PointLight& _light,*/ World const &_world) {
    ...
    for (auto i : std::views::iota(0ULL, m_threads.size())) {
        ...
        auto end_y =
            std::min<size_t>(start_y + NUM_ROWS_PER_THREAD, m_image_height);
